        ...
        .map(
          (category) => `
            ...
            ${groupedItems[category]
              .map(
                (item) => `
                <tr data-id="${item.id}" class="${parseInt(item.stockQuantity) < 10 ? 'bg-warning bg-opacity-10' : ''}">
            ...
            fetch(`http://localhost:5000/api/items/${itemId}`, {
              ...
              .catch((error) => {
                ...
                toast.innerHTML = `
                  <div class="d-flex">
                    <div class="toast-body">
                      <i class="fas fa-exclamation-circle me-2"></i>Error deleting item: ${error.message}
                    </div>
                    <button type="button" class="btn-close me-2 m-auto" data-bs-dismiss="toast" aria-label="Close"></button>
                  </div>
                `;
